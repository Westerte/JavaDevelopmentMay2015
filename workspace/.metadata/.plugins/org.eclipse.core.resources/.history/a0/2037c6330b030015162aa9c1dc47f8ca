...
public class AirlineAirplainsFinder {
	...
	public static List<Airplane> findByFuelConsumption(int minRange, int maxRange) throws LogicalException {
		...
		for (Airplane airplane : airplanes) {
			if (airplane.getFuelConsumption() > minRange  && airplane.getFuelConsumption() < maxRange) {
				result.add(airplane);
			}
		}
		return result;
