...
public class Auction extends Thread {
	...
	private boolean checkGuilty() {
		...
		while(blockedParticipants.containsKey(Thread.currentThread()) && !isClosed()) {
			if(!isBlocked) {
				phaser.arriveAndDeregister();
				isBlocked = true;
				System.out.println(String.format("Поток %s является должником", 
		...
		if(isBlocked) {			
			System.out.println(String.format("Потоку %s снова доступен аукцион" , 
					Thread.currentThread().getName()));		
			return true;
		}
		return false;
	...
	private void betting(Participant participant) {
		currentLot.register();
		System.out.println(String.format("Поток: %s зарегистрировался на лот: %s",  
		...
		while(decigion) {	
			...
			try {
				...
				System.out.println(String.format("Поток: %s сделал ставку: %d", 
	...
	private void finishLot() { 
		if(currentLot.getWinner() != null) {
			...
			if(currentLot.isConfirmed()) {
				System.out.println(String.format("Поток %s заплатил, лот является завершённым", 
						currentLot.getWinner().getName()));
				finishedLots.add(currentLot);
				updateBlockedParticipants();
			} else {		
				updateBlockedParticipants();
				System.out.println(String.format("Поток %s не заплатил, лот будет переигран", 
						currentLot.getWinner().getName()));						
